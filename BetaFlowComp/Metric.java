...
/*           Metric class:  Metric for staggered mesh                   */ 
...
public class Metric {
   ...
   public double D1u[][], D2u[][], D3u[][], D4u[][], D5u[][], D6u[][], D7u[][], D8u[][];
   public double D0u[][], D1v[][], D2v[][], D3v[][], D4v[][], D5v[][], D6v[][], D7v[][];
   public double D8v[][], D0v[][], D1P[][], D2P[][], D3P[][], D4P[][], D5P[][], D6P[][];
   public double D7P[][], D8P[][], D0P[][];  
   public double c1[][], c2[][], c3[][], C1w[][], C2w[][], C2s[][], C3s[][];
   
   void metric(Grid gr){
      int    mx=gr.getMx(), my=gr.getMy(), i, j, ip, jp, im, jm, mxm=mx-1, mym=my-1;
      ...
      D1u=new double[mx][mym]; D2u=new double[mx][mym]; D3u=new double[mx][mym]; 
      D4u=new double[mx][mym]; D5u=new double[mx][mym]; D6u=new double[mx][mym];
      D7u=new double[mx][mym]; D8u=new double[mx][mym]; D0u=new double[mx][mym];
      D1v=new double[mxm][my]; D2v=new double[mxm][my]; D3v=new double[mxm][my];
      D4v=new double[mxm][my]; D5v=new double[mxm][my]; D6v=new double[mxm][my];
      D7v=new double[mxm][my]; D8v=new double[mxm][my]; D0v=new double[mxm][my];
      D1P=new double[mxm][mym];D2P=new double[mxm][mym];D3P=new double[mxm][mym];
      D4P=new double[mxm][mym];D5P=new double[mxm][mym];D6P=new double[mxm][mym];
      D7P=new double[mxm][mym];D8P=new double[mxm][mym];D0P=new double[mxm][mym];
  
      c1 =new double[mx][my];  c2 =new double[mx][my];  c3 =new double[mx][my];
      C1w=new double[mx][mym]; C2w=new double[mx][mym];
      C2s=new double[mxm][my]; C3s=new double[mxm][my];
	  
      double XIx[][] =new double[mx][my];  double XIy[][] =new double[mx][my];
      double ETAx[][]=new double[mx][my];  double ETAy[][]=new double[mx][my];
      ...
      for(i = 0; i<mx; i++){
         for(j = 0; j<my; j++){
            if(i == 0){
               Xxi=0.5*(-3.0*gr.x[0][j]+4.0*gr.x[1][j]-gr.x[2][j]);
               Yxi=0.5*(-3.0*gr.y[0][j]+4.0*gr.y[1][j]-gr.y[2][j]);
            }else if(i == mxm){
               Xxi=0.5*(gr.x[mx-3][j]-4.0*gr.x[mx-2][j]+3.0*gr.x[mxm][j]);
               Yxi=0.5*(gr.y[mx-3][j]-4.0*gr.y[mx-2][j]+3.0*gr.y[mxm][j]);
            }else{
               Xxi=-0.5*(gr.x[i-1][j]-gr.x[i+1][j]);
               Yxi=-0.5*(gr.y[i-1][j]-gr.y[i+1][j]);
            }
            if(j == 0){
               Xeta=0.5*(-3.0*gr.x[i][0]+4.0*gr.x[i][1]-gr.x[i][2]);
               Yeta=0.5*(-3.0*gr.y[i][0]+4.0*gr.y[i][1]-gr.y[i][2]);
            }else if(j == mym){
               Xeta=0.5*(gr.x[i][my-3]-4.0*gr.x[i][my-2]+3.0*gr.x[i][mym]);
               Yeta=0.5*(gr.y[i][my-3]-4.0*gr.y[i][my-2]+3.0*gr.y[i][mym]);
            }else{
               Xeta=-0.5*(gr.x[i][j-1]-gr.x[i][j+1]);
               Yeta=-0.5*(gr.y[i][j-1]-gr.y[i][j+1]);
            }
